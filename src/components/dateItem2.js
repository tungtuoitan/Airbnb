import React, {
  ...
  import { pickDate,pickFirstDate,pickLastDate } from "../reducer/dateItemSlice";
...
import { setCurrentPopUp } from "../reducer/searchSlice";
  ...
  function DateItem2({ item, available }) {
    let isSearchPopUpOpen = useSelector((state) => state.isSearchPopUpOpen);
    let currentPopUp = useSelector((state) => state.searchSlice.currentPopUp);
    ...
    // logic of callendar
    const firstDays = firstDate[0] + (firstDate[1]+1) * 40 + firstDate[2] * 3650;
    ...
    const handleOnClick = (item) => {
      if (currentDays > todayDays) {
        if(currentPopUp==='checkin'){
            dispatch(pickFirstDate(dateArr));
            // dispatch(setCurrentPopUp('checkout'))
        }else if(currentPopUp==='checkout'){
            dispatch(pickLastDate(dateArr))
