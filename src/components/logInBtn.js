...
export default function LogInBtn() {
  ...
  return (
    ...
    to={isLogged ? '/account-settings':'/login'}>
      <svg
        ...
        ${path==='/login' || path==='/account-settings' ? 'fill-main':'fill-current opacity-60'}`}
      >
      ...
      <span className={`${path==='/login' || path==='/account-settings' ? 'text-black222 font-semibold':''}`}>Log in</span>
    </Link>
  );
}
