...
export default function LogInBtn() {
  ...
  return (
    <Link className=" flex items-center flex-col gap-c3  mt-c12 mb-2 w-1/5" to='/login'>
      <svg
        ...
        ${path==='/login' || path==='/xxx' ? 'fill-main':'fill-current opacity-60'}`}
      >
      ...
      <span className={`${path==='/login'? 'text-black222 font-semibold':''}`}>Log in</span>
    </Link>
  );
}
